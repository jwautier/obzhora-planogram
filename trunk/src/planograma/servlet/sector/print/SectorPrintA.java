...
public class SectorPrintA extends HttpServlet {
	public static final String URL = UrlConst.URL_SECTOR_PRINT_A;

	private static final Logger LOG = Logger.getLogger(SectorPrintA.class);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			final List<Rack> list = rackHModel.listA(userContext, code_sector);
			...
			for (int index = 0; index < rackList.size(); index++) {
				...
				if (rack2D.getRack().getType_rack() != ETypeRack.DZ) {
					...
					EStateRack state;
					if (rackState.getDate_complete() == null || rackState.getDate_active().getTime() >= rackState.getDate_complete().getTime()) {
						state = EStateRack.A;
					} else {
						state = EStateRack.PC;
					}
					...
					if (state == EStateRack.A) {
						table.addCell(new PdfPCell(new Paragraph(FormattingUtils.datetime2String(rackState.getDate_active()), font)));
					} else {
					...
					if (state == EStateRack.A) {
						fullName = userModel.getFullName(userContext, rackState.getUser_active());
					} else {
						fullName = userModel.getFullName(userContext, rackState.getUser_complete());
					}
